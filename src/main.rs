...
#[derive(Parser, Debug)]
#[command(
    name = "mnml-db-mariadb",
    version,
    about = "MariaDB / MySQL query viewer for mnml"
)]
struct Cli {
    ...
    /// a pane (`:host.launch mnml-db-mariadb --blit /tmp/x.sock`).
...
/// Redact `:<pass>@` in a MySQL/MariaDB DSN for terminal display.
...
mod tests {
    ...
    fn scrub_dsn_hides_password() {
        let s = scrub_dsn("mysql://api:hunter2@db.example.com:3306/api");
        assert_eq!(s, "mysql://api:****@db.example.com:3306/api");
    }

    #[test]
    fn scrub_dsn_without_password_is_idempotent() {
        let s = scrub_dsn("mysql://localhost:3306/test");
        assert_eq!(s, "mysql://localhost:3306/test");
    }
}
