//! Config file at `~/.config/mnml-db-mariadb.toml`. First run writes
...
pub struct Connection {
    ...
    pub name: String,
    /// MariaDB connection string — `mariadbql://user:pass@host:port/db`.
    /// Anything `tokio-mariadb` accepts is fine. The file should be
...
impl Config {
    pub const EXAMPLE: &'static str = r##"# mnml-db-mariadb config. Edit and re-run.
#
...
[[connections]]
name = "local"
dsn = "mariadbql://mariadb@localhost:5432/mariadb"

# [[connections]]
# name = "prod-api"
# dsn = "mariadbql://api_readonly:${PROD_DB_PASS}@db.prod.example.com:5432/api"

# [[connections]]
# name = "staging"
# dsn = "mariadbql://api_readonly:${STAGING_DB_PASS}@db.staging.example.com:5432/api"
"##;
...
"##;
    ...
    fn expand_env_substitutes_known_vars() {
        ...
        let s = expand_env("mariadbql://u:${MNML_DB_PG_TEST}@h/d");
        assert_eq!(s, "mariadbql://u:hunter2@h/d");
        unsafe { std::env::remove_var("MNML_DB_PG_TEST") };
    }

    #[test]
    fn expand_env_leaves_unknown_vars_in_place() {
        let s = expand_env("mariadbql://u:${DEFINITELY_NOT_SET_zzz}@h/d");
